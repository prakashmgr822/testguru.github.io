                ...
                @foreach($item->questions as $i => $question)
                    <tr>
                        ...
                        <td class="text-center">
                            <a target="_blank" href="{{route('questions.edit',$question->id)}}"
                               class="btn btn-sm btn-clean btn-icon btn-hover-danger">
                                <i class="fas fa-edit" style="font-size: 17px"></i></a>
                            <form class="d-inline"
                                  action="{{ route('questionDelete',['question_id' => $question->id, 'test_id' => $item->id]) }}"
                                  method="POST" onclick="return confirm('Are you sure?')">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm btn-clean btn-icon">
                                    <i class="fas fa-times-circle close " style="font-size: 20px; color: black"></i>
                                </button>
                            </form>
                        </td>
                    </tr>
